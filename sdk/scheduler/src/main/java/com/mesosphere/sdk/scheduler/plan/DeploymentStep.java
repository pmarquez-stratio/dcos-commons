...
public class DeploymentStep extends AbstractStep {
    ...
    private static Set<Protos.TaskInfo> getTaskInfos(Collection<OfferRecommendation> recommendations) {
        return recommendations.stream()
                ...
                .filter(taskInfo -> !taskInfo.getTaskId().getValue().isEmpty())
                .collect(Collectors.toSet());
    }
    ...
    public synchronized void updateOfferStatus(Collection<OfferRecommendation> recommendations) {
        ...
        logger.info("Step '{} [{}]' is now waiting for updates for task IDs: {}",
                getName(), getId(), tasks.keySet().stream().map(Protos.TaskID::getValue).collect(Collectors.toList()));
